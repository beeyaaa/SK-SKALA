...
// 검색어를 입력할 때마다 목록을 다시 그린다./* TODO: [검색어 입력시 목록 재 출력] 수강생 구현 #3 */
searchInput.addEventListener('input', renderCars)


// 판매 상태를 바꿀 때마다 목록을 다시 그린다. /* TODO: [판매 상태 변경시 목록 재 출력] 수강생 구현 #4 */
...
function getCarFromForm() {
  ...
  if (yearText === "" || year < 1990 || year > maxYear) {
    alert(`연식은 1990년부터 2026년(현재년도) 사이로 입력하세요.`)
    return null
  }
...
function deleteCar(id) {
  ...
  const confirmDelete = confirm("선택한 차량을 삭제할까요?")
  if (!confirmDelete) {
    return
  }
